...
### Превью картинок и скачивание файлов (2026-09-14)
- `getPreviews()` (`attachments.js`) теперь отдаёт данные вложения:
  `dataUrl` для картинок, полный `text` для текстовых (обрезка до
  `MAX_TEXT_LENGTH` применяется только при сборке `prompt` в `getTextContext()`).
- `render.js`: картинка с `dataUrl` — кликабельная миниатюра `.msg__img-thumb`;
  текстовый файл с `text` — чип с кнопкой скачивания
  `.msg__attachment-download` (скачивание через `Blob` + `URL.createObjectURL`).
- `index.html` + `app.js`: lightbox `#lightbox` (`#lightbox-img`,
  `#lightbox-close`), открытие по клику на миниатюру, закрытие — клик по фону,
  крестик или `Esc`; при «Новом чате»/очистке закрывается.
- `state.js`: `stripAttachmentData()` вычищает `dataUrl`/`text` из вложений при
  `persistBranches()` — данные файлов не сериализуются в `localStorage` и не
  накапливаются. Данные живут только в памяти сессии (после reload доступны лишь
  метаданные). На бэкенде файлы не хранятся (base64 идёт в памяти в DeepSeek),
  поэтому накопления на сервере нет.

...
доработка `POST /chat/facts` под Sticky Facts (см. дрейф фактов в
`experiments/context-strategies/report.md`): append-only-семантика,
программная дедупликация, извлечение из всего диалога, структурированный
вывод.
